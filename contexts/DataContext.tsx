  ...
  const getBetData = useCallback(
    async (betId: number) => {
      ...
      return {
        ...
        currentARatio: toNumber(betData.currentARatio.toString()) / 10,
        currentBRatio: parseInt(betData.currentBRatio.toString()) / 10,
        currentTotalAAmount:
          parseInt(betData.currentTotalAAmount.toString()) / 10,
        currentTotalBAmount:
          parseInt(betData.currentTotalBAmount.toString()) / 10,
      };
    },
    [polymarket]
  );
    ...
    async (userAddress: Address) => {
      ...
      return {
        abets: userBetsA.map((bet: UserBet) => ({
          amount: parseInt(bet.amount.toString()) / 10,
          averageRatio: parseInt(bet.averageRatio.toString()) / 10,
